...
parser.add_argument(
    "-i",
    help="Input Fasta file (default = results/secuencias_n.fasta)",
    default="results/secuencias_n.fasta",
)
parser.add_argument(
    "-o",
    help="Output file (default = results/sequence.orf)",
    default="results/secuencias.orf",
)
parser.add_argument(
    "-opc", help="Options orf or domain_analysis(default = orf)", default="orf"
)
...
def run_emboss_getorf(input_fasta, output_aa):
    subprocess.run(
        ["getorf", "-sequence", input_fasta, "-outseq", output_aa], check=True
    )
